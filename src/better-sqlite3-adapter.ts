 ...
 * - Modified the adapter factory to use an arbitrary `Database` input.
 * - Fixing linting, formatting, and type checking to conform to our standards.
 *
...
const packageName = 'prisma-generic-better-sqlite3';
...
class BetterSQLite3Queryable<ClientT extends Database.Database> implements SqlQueryable {
    public readonly provider = 'sqlite';
    public readonly adapterName = packageName;
    ...
    private performIO(query: SqlQuery): Promise<BetterSQLite3ResultSet> {
        try {
            ...
            const resultSet = {
                declaredTypes: columns.map((column: any) => column.type),
...
class BetterSQLite3Transaction
    extends BetterSQLite3Queryable<Database.Database>
    implements Transaction
{
    constructor(
        client: Database.Database,
        public readonly options: TransactionOptions,
        public readonly unlockParent: () => void,
    ) {
        super(client);
    }
...
export class PrismaBetterSQLite3AdapterFactory implements SqlMigrationAwareDriverAdapterFactory {
    ...
    constructor(
        ...
        config.database.defaultSafeIntegers(true);
        config.shadowDatabase?.defaultSafeIntegers(true);
    }

    public connect(): Promise<SqlDriverAdapter> {
        return Promise.resolve(new PrismaBetterSQLite3Adapter(this.config.database));
    }

    public connectToShadowDb(): Promise<SqlDriverAdapter> {
        const shadowDatabase: Database.Database =
            this.config.shadowDatabase || this.createShadowDatabase();

        return Promise.resolve(new PrismaBetterSQLite3Adapter(shadowDatabase));
    }

    private createShadowDatabase(): Database.Database {
        const shadowDatabase = new Database(':memory:');
        shadowDatabase.defaultSafeIntegers(true);
...
class UnexpectedTypeError extends Error {
    public override readonly name = 'UnexpectedTypeError';
    constructor(value: unknown) {
        const type = typeof value;
        const error = type === 'object' ? JSON.stringify(value) : String(value);
        super(`unexpected value of type ${type}: ${error}`);
    }
}
